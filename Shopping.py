...
def save_file():
    ...
    str = repr(basket_kala)
    file.write(str)
    file.close()


def save_file_counter(counter_faktor):
    file = open('counter.txt', 'w')
    str = repr(counter_faktor)
    # print("in the save file function", counter_faktor)
    file.write(str)
    file.close()


def save_file_faktor():
    file = open('faktor.txt', 'a')
    str = repr(list_faktor)
    file.write(str)
    file.close()
...
def search_kala_def():
    list_search = []
    search_kala = input("Inter Name Kala or Code Kala : For END of Buy Typed 'exit' Or return to menu: typed 'menu'")
    if search_kala == "exit":
...
def menu_def():
    ...
    print("4- Save")
    print("5- Inter Faktor Number to print Faktor")
    ...
    if choose_menu_item == 5:
        list_test = load_faktor()
        while True:
            temp_num_faktor = input("Please Inter Number Faktor to Print: ")
            print(list_test)
            if temp_num_faktor == "exit":
                menu_def()
            elif temp_num_faktor not in list_test:
                print("Number Faktor not exist, Please try again")
            else:
                start = list_test.index(temp_num_faktor)
                end = list_test.index("end", start)
...
def buy_kala_test(counter_faktor):
    while True:
        ...
        print(number_faktor)
        total = 0
        while True:#Start Buy
            flag = True
            globals()[f"dic{counter_faktor}"] = {}
            list_search, search_kala = search_kala_def()
            if search_kala == "menu": # for print last faktor
                save_file()
                save_file_counter(counter_faktor)
                menu_def()
            if search_kala == "exit":
                save_file()
                save_file_counter(counter_faktor)
                if len(basket_kala) != 0:
                    print("Faktor Buy:", f'Shomare_Faktor: {basket_kala[-1]["number_faktor"]}')

                    for i in range(len(basket_kala)):
                        if basket_kala[i]["number_faktor"] == number_faktor:
                            if number_faktor not in list_faktor:
                                print("Not exist")
                                list_faktor.append(number_faktor)
                            for key, value in basket_kala[i].items():
                                if key != "number_faktor" and key != "total-faktor":
                                    list_faktor.append(f'{key}:{value}')
                                    print(key, ': ', value)
                            if i == len(basket_kala) - 1:
                                list_faktor.append(f'total-price-faktor: {basket_kala[i]["total-faktor"]}')
                                list_faktor.append("end")
                                print("list_faktor = ", list_faktor)
                    save_file_faktor()
                    list_faktor.clear()
                    ...
                    if list_search[0] == list_kala[i]["id"]:
                        while True:
                            ...
                            if numbers_kala <= list_kala[i]["mojodi"]:
                                ...
                                print(list_kala[i])
                                print(f"basket_kala = {basket_kala}")
                                break
                            if numbers_kala > list_kala[i]["mojodi"]:
...
counter_faktor = load_file_counter()
load_file_db()
while True:
    menu_def()